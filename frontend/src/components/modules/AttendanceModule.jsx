...
import { CheckCircle, Clock, XCircle, UserCheck, Calendar, Activity, RefreshCw } from 'lucide-react';

const INITIAL_MOCK_ROSTER = [
...
export default function AttendanceModule() {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setRoster(data.roster || INITIAL_MOCK_ROSTER);
        setLogs(data.logs || INITIAL_MOCK_LOGS);
      } else {
        throw new Error('Fetch failed');
      }
    } catch (err) {
      console.warn('Backend unavailable, using local mock data.', err);
      setRoster(INITIAL_MOCK_ROSTER);
  ...
  const handleStatusChange = async (userId, newStatus) => {
    ...
    try {
      ...
      console.warn('Backend status update failed, logging locally.', err);
...
const styles = {
  ...
  rosterRow: {
    ...
    backgroundColor: 'rgba(109, 76, 65, 0.02)',
